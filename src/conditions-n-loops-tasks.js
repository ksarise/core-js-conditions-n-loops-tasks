...
function isIsoscelesTriangle(a, b, c) {
  return a + b > c && b + c > a && c + a > b && (a === b || b === c || a === c);
}
...
function getSpiralMatrix(size) {
  ...
  while (colStart <= colEnd) {
    for (let col = colStart; col <= colEnd; col += 1) {
      matrix[startRow][col] = linkNum;
      linkNum += 1;
    }
    startRow += 1;

    for (let row = startRow; row <= endRow; row += 1) {
      matrix[row][colEnd] = linkNum;
      linkNum += 1;
    }
    colEnd -= 1;

    for (let col = colEnd; col >= colStart; col -= 1) {
      matrix[endRow][col] = linkNum;
      linkNum += 1;
    }
    endRow -= 1;

    for (let row = endRow; row >= startRow; row -= 1) {
      matrix[row][colStart] = linkNum;
      linkNum += 1;
    }
    colStart += 1;
  }
...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < matrix.length; i += 1) {
    for (let j = 0; j < matrix[i].length; j += 1) {
      copyMatrix[j][matrix.length - 1 - i] = result[i][j];
    }
  }
...
function getNearestBigger(number) {
  ...
  while (digits[swap] <= digits[swap - 1]) {
    swap -= 1;
  }

  const i = swap;
  if (i !== 0) {
    for (swap = digits.length - 1; swap >= i; swap -= 1) {
      if (digits[i - 1] < digits[swap]) {
        const prev = digits[i - 1];
        digits[i - 1] = digits[swap];
        digits[swap] = prev;
        break;
      }
    }
  ...
  swap = i;
  for (let j = digits.length - 1; swap < j; j -= 1) {
    const prev = digits[swap];
    digits[swap] = digits[j];
    digits[j] = prev;
    swap += 1;
  }

  let result = 0;
  for (swap = 0; swap < digits.length; swap += 1) {
    result = result * 10 + digits[swap];
  }
